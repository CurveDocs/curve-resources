...
- Lending markets are permissionless; any assets in combination with crvUSD can be used. This means users can borrow against tokens like $CRV, $DOGE, etc. You name it. The only requirement is a proper oracle[^1]. Although, before creating a lending market, proper parameters should be simulated.
...
-   :fontawesome-solid-money-bill-1: **Borrowers**
    ...
    Borrowers are the ones borrowing assets. To do so, they create a loan and put up some collateral to borrow against. In exchange for borrowing, they pay a certain [borrow rate](#borrow-apr) which soley depends on the [utilization](#utilization-rate){ data-preview } of the market.
...
-   :material-bank: **Lenders**
    ...
    Lenders are lending out their assets to the borrower by deposit their assets into an slightly modified [ERC-4626 vault](https://ethereum.org/en/developers/docs/standards/tokens/erc-4626/) form where they can be borrowed. In exchange for supplying their assets, they receive an [lending rate](#lend-apy).
...
!!!info "Long and Short Markets"
    When creating new markets, one of the parameters to input is a `name`, which will be used for display on the UI.
...
`Supply` represents the underlying token of the vault which can be supplied and `Lend APY` the annualized rate for doing so.

Additionally, vaults can have gauges, which are eligible to receive CRV emissions once they're added to the GaugeController. These rewards will show up in the `Rewards APR / CRV + Incentives` column if there are any.
...
LLAMMA (Lending-Liquidating AMM Algorithm) is a fully functional two-token AMM containing the collateral and borrowable token, which is **reponsible for the liquidation mechanism** behind all lending markets. For a more detailed documentation, please refer to the [technical docs](https://docs.curve.fi/crvUSD/amm/).

When creating a new loan, the put-up **collateral will be deposited into a specified number of bands across the AMM**. Unlike regular liquidation, which have a single liquidation price, LLAMMA has multiple liquidation ranges (represented by the bands) and continiously liquidates the colalteral if needed. All bands have a lower and upper price limit, each representing a "small liquidation range". The user's total liquidation range is represented by the upper price of the highest band to the lower price of the lowest band. 

A loan only enters soft-liquidation mode once the price of the collateral asset is within a band. If the price is outside of the bands, there is no need to partially liquidate and therefor not in soft-liquidation.

The AMM works in a way that the collateral price within the AMM and the "regular price" are treated a bit differently. If the price falls into a band, price are adjusted in a way that external arbitrageurs are incentivized to sell the collateral token and buy the borrowable token in the band. So, **if the price is within a band, the user's collateral will be sold for the borrowable token**, meaning the users collateral is now a combination of both tokens. This happens for each individual band the user has liquidity deposited into.  

**This liquidation process does not only happen when prices fall but also when they rise again**. So if the collateral in a band has been fully converted into the borrowable token and the collateral price rises again, your earlier sold-off collateral will be bought up again.

*In short: External traders will soft-liquidate your collateral when the collateral token's price is falling and de-liquidate it again when prices rise again.*
...
!!!warning "Losses in Soft-Liquidation"
    ...
    [^2]: The borrow rate the user is paying for borrowing assets is the only "loss" that occurs.
...
A band is a price range, with an upper and lower price limit. If the price of the collateral is between the two prices of a band, that particular band is likely to be liquidated.
...
As seen above, there are many bands with different price ranges. The light grey represents the collateral token, in this example ETH. As seen, the bands below the price of the collateral are fully in ETH as there is no need to liquidate them, because the price is higher. The dark grey represents the borrowable token, in this case crvUSD. Becuase the price of ETH is within the band to the far right, deposited collateral (ETH) is converted into the borrowed token (crvUSD). In this case, the band consists of ETH and crvUSD. If the price further falls, all collateral in the band will be fully sold into crvUSD and the band to the left will "undergo" soft-liquidation.
...
$$\text{borrow rate} = \text{rate}_{\text{min}} \cdot \left(\frac{\text{rate}_{\text{max}}}{\text{rate}_{\text{min}}}\right)^{\text{utilization}}$$
...
$$\text{lendAPY} = \left(1 + \frac{lendAPR}{86400 \cdot 365}\right)^{86400 \cdot 365} - 1$$


!!!note "Rate Denomination"
    NOTE: `lendAPR` and `lendAPY` used in the calculations is denominated in decimals, not percentage. `5% -> 0.05`
